...
double lx=0,ly=0,lz=-1; //The eye is initially pointed to the origin 
float jump = 3; // the strafe should jump by 0.5
int move=1;
int light=1;
...
static void zebracrossing()
{
	...
	glBegin(GL_QUADS);
	glNormal3f(0,1,0);
	glTexCoord2f(0,1); glVertex3f(-62,-8.5,-15);
    glTexCoord2f(0,0); glVertex3f(-62,-8.5,15);         //a long road
    glTexCoord2f(1,0); glVertex3f(62,-8.5,15);
    glTexCoord2f(1,1); glVertex3f(62,-8.5,-15); 
    glEnd();
    glDisable(GL_POLYGON_OFFSET_FILL);
    glDisable(GL_TEXTURE_2D);
}
...
void special(int key,int x,int y)
{
   ...
   if (key == GLUT_KEY_RIGHT)
    ...
    else if (key == GLUT_KEY_UP)
    {
         ...
         Ex += (float)sin(lx)*jump;
		 Ez -= (float)cos(lz) *jump;
